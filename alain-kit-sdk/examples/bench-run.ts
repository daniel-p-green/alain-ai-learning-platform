...
async function runOne(t: Target, outRoot: string) {
  ...
  if (res.notebook) fs.writeFileSync(path.join(dir, 'notebook.ipynb'), JSON.stringify(res.notebook, null, 2));
...
(async () => {
  ...
  for (const t of TARGETS) {
    if (t.type === 'poe' && !t.apiKey) { console.log(`Skipping ${t.name} — POE_API_KEY not set`); continue; }
    try {
