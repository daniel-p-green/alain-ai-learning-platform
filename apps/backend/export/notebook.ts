...
export function buildNotebook(
  ...
  const cells: NBCell[] = [];
  const providerId = (meta.provider || 'openai-compatible').toLowerCase();
  const isPoe = providerId === 'poe';
  const isHuggingFace = providerId === 'huggingface';
  const providerBase = isPoe
    ? 'https://api.poe.com/v1'
    : isHuggingFace
      ? 'https://api-inference.huggingface.co/v1'
      : 'http://localhost:1234/v1';
  const providerDisplay = isHuggingFace
    ? 'Hugging Face Inference'
    : (isPoe ? 'Poe' : meta.provider);
  ...
  // 1) Intro
  const intro: string[] = [
    `> Generated by ALAIN (Applied Learning AI Notebooks) — ${creationDate}.\n`,
    `# ${meta.title}\n`,
    `\n`,
    `${meta.description}\n`,
    `\n`,
    `> Provider: \`${providerDisplay}\`  •  Model: \`${meta.model}\`\n`,
    `Runtime: ${providerDisplay}\n`,
    `\n`,
  ...
  // Provider base URL setup handled via providerBase constant above
  ...
  const envExampleSource = [
    ...
    `ALAIN_PROVIDER=${providerId}\n`,
    `OPENAI_BASE_URL=${providerBase}\n`,
  ];
  if (isPoe) {
    envExampleSource.push("POE_API_KEY=your_poe_key_here\n");
    envExampleSource.push("OPENAI_API_KEY=\n");
    envExampleSource.push("HF_TOKEN=\n");
  } else if (isHuggingFace) {
    envExampleSource.push("HF_TOKEN=your_hf_token_here\n");
    envExampleSource.push("# OPENAI_API_KEY falls back to HF_TOKEN when left blank\n");
    envExampleSource.push("OPENAI_API_KEY=\n");
    envExampleSource.push("POE_API_KEY=\n");
  } else {
    envExampleSource.push("OPENAI_API_KEY=your_api_key_here\n");
    envExampleSource.push("POE_API_KEY=\n");
    envExampleSource.push("HF_TOKEN=\n");
    envExampleSource.push("# For local models (LM Studio/Ollama), any non-empty string works for API_KEY\n");
  }
  cells.push({
    cell_type: "code",
    metadata: {},
    source: envExampleSource,
    outputs: [],
    execution_count: null,
  });
  ...
  const envSource = [
    "# Configure OpenAI-compatible client (maps provider-specific keys)\n",
    "import os\n",
    "from getpass import getpass\n",
    ...
    "except Exception:\n",
    "  _poe = None; _openai = None; _hf = None\n",
    `PROVIDER = "${providerId}"  # 'poe', 'openai-compatible', or 'huggingface'\n`,
    `os.environ.setdefault("ALAIN_PROVIDER", PROVIDER)\n`,
    `os.environ.setdefault("ALAIN_MODEL", "${meta.model}")\n`,
    `default_base = "${providerBase}"\n`,
    "base = os.environ.get('OPENAI_BASE_URL') or default_base\n",
    "os.environ.setdefault('OPENAI_BASE_URL', base)\n",
    "if PROVIDER == 'poe':\n",
    "  poe_key = _poe or os.getenv('POE_API_KEY')\n",
    "  if poe_key:\n",
    "    os.environ.setdefault('OPENAI_API_KEY', poe_key)\n",
    "  else:\n",
    "    os.environ['OPENAI_API_KEY'] = getpass('Enter POE_API_KEY (hidden): ')\n",
    "  os.environ['OPENAI_BASE_URL'] = 'https://api.poe.com/v1'\n",
    "elif PROVIDER == 'huggingface':\n",
    "  token = _hf or os.getenv('HF_TOKEN')\n",
    "  os.environ['OPENAI_BASE_URL'] = 'https://api-inference.huggingface.co/v1'\n",
    "  if token:\n",
    "    os.environ.setdefault('OPENAI_API_KEY', token)\n",
    "  else:\n",
    "    os.environ['OPENAI_API_KEY'] = getpass('Enter HF_TOKEN (hidden): ')\n",
    "else:\n",
    "  if _openai and not os.environ.get('OPENAI_API_KEY'):\n",
    "    os.environ['OPENAI_API_KEY'] = _openai\n",
    "  poe_key = _poe or os.getenv('POE_API_KEY')\n",
    "  if poe_key and not os.environ.get('OPENAI_API_KEY'):\n",
    ...
    "if not os.environ.get('OPENAI_API_KEY'):\n",
    "  label = 'HF_TOKEN' if PROVIDER == 'huggingface' else ('POE_API_KEY' if PROVIDER == 'poe' else 'OPENAI_API_KEY')\n",
    "  os.environ['OPENAI_API_KEY'] = getpass(f'Enter {label} (hidden): ')\n",
    "# OPENAI_BASE_URL and OPENAI_API_KEY environment variables are now set\n",
  ];
