...
export const publishTutorial = api<{ id: number; visibility: Visibility }, { success: boolean; share_slug?: string | null }>(
  ...
  async (req, ctx) => {
    ...
    // Admin override via ADMIN_USER_IDS (comma-separated user IDs)
    const admins = (process.env.ADMIN_USER_IDS || '').split(',').map(s=>s.trim()).filter(Boolean);
    const isAdmin = admins.includes(userId);
    if (!isAdmin && exists.author_id && exists.author_id !== userId) throw APIError.permissionDenied('tutorial not owned by user');
