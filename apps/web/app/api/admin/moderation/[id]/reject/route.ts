...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    nb.metadata = { ...(nb.metadata || {}), moderation: 'rejected', published: false };
    const sha = await getFileSha(owner, repo, path, branch);
    const res = await putFile({ owner, repo, branch, path, content: JSON.stringify(nb, null, 2), message: `Moderation: reject ${params.id}`, sha });
