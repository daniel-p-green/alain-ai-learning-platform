...
export const publishNotebookApi = api<{
  ...
  async (req) => {
    ...
    const existing = await tutorialsDB.queryRow<{ id: number }>`SELECT id FROM generated_notebooks WHERE file_path = ${req.file_path}`;
...
export const publishLessonApi = api<{
  ...
  async (req) => {
    if (!req.file_path) throw APIError.invalidArgument('file_path required');
    if (!['private','public','unlisted'].includes(req.visibility)) throw APIError.invalidArgument('invalid visibility');
    const existing = await tutorialsDB.queryRow<{ id: number }>`SELECT id FROM generated_lessons WHERE file_path = ${req.file_path}`;
