...
// Import existing local notebooks under content/notebooks into the catalog index.
// This lets us seed "base" notebooks without regenerating them.
export const importLocalNotebooks = api<{
  visibility?: Visibility; // default 'private'
  root?: string; // defaults to process.env.ALAIN_STORAGE_ROOT || 'content'
  ...
  async (req, ctx) => {
    ...
    const storageRoot = path.resolve(process.cwd(), req.root || process.env.ALAIN_STORAGE_ROOT || 'content');
    ...
    function deriveFromPath(absPath: string): { provider: string; model: string; difficulty: 'beginner'|'intermediate'|'advanced'|null } {
      // Expect content/notebooks/{provider}/{model}/[difficulty]/...
