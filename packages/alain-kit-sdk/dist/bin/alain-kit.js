...
function computeReadability(notebook) {
...
async function main() {
    ...
    const difficulty = parseArg('--difficulty', 'beginner');
    ...
    if (remixPath) {
        ...
        res = await kit.generateNotebook({
            ...
            difficulty,
            maxSections,
            customPrompt: sectionTokens ? { maxTokens: sectionTokens } : undefined
        });
    }
    if (!res.success) {
    ...
    // Track the best section-count insight we have so downstream metadata/tests agree.
    let sectionCount = Array.isArray(res.sections) ? res.sections.length : (typeof res.sectionCount === 'number' ? res.sectionCount : undefined);
    try {
        ...
        const inferredSections = cells.length ? Math.max(4, Math.round(cells.length / 6)) : 6;
        sectionCount = sectionCount ?? inferredSections;
        const perSection = {
        ...
        meta.estimated_time_minutes = { min: estMin, max: estMax };
    ...
    const metrics = {
        ...
        markdownCells: res.notebook?.cells?.filter((c) => c.cell_type === 'markdown').length ?? 0,
        codeCells: res.notebook?.cells?.filter((c) => c.cell_type === 'code').length ?? 0,
    ...
    try {
        const est = res.notebook?.metadata?.estimated_time_minutes;
        if (est?.min && est?.max) {
...
// --- Remix helpers ---
function asText(src) { return Array.isArray(src) ? src.join('') : String(src || ''); }
function inferTitle(nb) {
...
function extractHeadings(nb, limit = 8) {
    const heads = [];
    const cells = Array.isArray(nb?.cells) ? nb.cells : [];
    for (const c of cells) {
        if (c?.cell_type !== 'markdown')
...
function extractPlainText(nb, maxChars = 8000) {
    let out = '';
    const cells = Array.isArray(nb?.cells) ? nb.cells : [];
    for (const c of cells) {
        if (c?.cell_type === 'markdown') {
...
//# sourceMappingURL=alain-kit.js.map