...
export class ColabValidator {
        ...
        this.ERROR_PATTERNS = [
            {
                ...
                fix: (source) => {
                    ...
                    return source.replace(pipCall, (_match, indent, tail) => {
                        const argsTail = tail || '';
            ...
            },
            {
                ...
                fix: (source) => source.replace(/os\.environ\["HF_TOKEN"\]\s*=\s*"YOUR_HF_TOKEN"/g, `if IN_COLAB:
    ...
    os.environ["HF_TOKEN"] = "YOUR_HF_TOKEN"`)
            },
            {
                ...
                fix: (source) => source.replace(/device_map="auto"/g, 'device_map="cuda:0" if torch.cuda.is_available() else "cpu"')
            }
        ];
            ...
            if (cell.cell_type === 'code') {
                ...
                this.ERROR_PATTERNS.forEach(pattern => {
                    ...
                    if (pattern.type === 'subprocess_pip') {
                        ...
                        const hasGuard = /if\s+IN_COLAB/.test(normalized) && (normalized.includes("run_line_magic('pip'") ||
    ...
    async applyFixes(notebook, issues) {
...
${result.issues.map((issue, i) => `${i + 1}. ${issue.type} (${issue.severity})\n   ${issue.description}`).join('\n\n')}
...
//# sourceMappingURL=colab-validator.js.map