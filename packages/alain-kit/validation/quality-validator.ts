...
export class QualityValidator {
  ...
  private readonly OPTIMAL_MARKDOWN_RANGE = [0.4, 0.7];
  ...
  private analyzeNotebook(notebook: any): QualityMetrics {
  ...
  private calculateQualityScore(metrics: any): number {
    ...
    if (metrics.structure.hasTitle) score += 10;
    if (metrics.structure.hasObjectives) score += 10;
    if (metrics.structure.hasSetup) score += 10;
    if (metrics.structure.hasAssessments) score += 10;
  ...
  private calculateReadingTime(tokens: number): number {
