...
export default function EditTutorialPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!res.ok) return setError("Failed to load tutorial");
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Edit Tutorial</h1>
      <p className="text-sm text-ink-600">Tutorial ID: {id}</p>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button onClick={() => router.push(`/tutorials/${id}`)} className="inline-flex items-center h-10 px-4 rounded-alain-lg bg-ink-200 text-ink-900 font-medium w-full sm:w-auto whitespace-nowrap">
          Cancel
        </button>
      </div>
    </div>
