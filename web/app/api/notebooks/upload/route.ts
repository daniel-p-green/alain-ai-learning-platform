...
export async function POST(req: Request) {
  ...
  let nb: any;
  try { nb = JSON.parse(buf.toString("utf-8")); } catch { return NextResponse.json({ error: "invalid JSON .ipynb" }, { status: 400 }); }
  ...
  const meta: NotebookMeta = { id, title, sourceType, sourceOrg, tags, createdAt: new Date().toISOString() } as any;
  try {
    ...
    const res = await putFile({ owner, repo, branch, path, content: JSON.stringify(nb, null, 2), message: `User upload notebook ${id}: ${title}`, sha: existingSha });
    putNotebook({ meta, nb });
