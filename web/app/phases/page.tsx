  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-600">Research → Design → Develop → Validate</p>
      </div>
      ...
      <div>
        ...
        <div className="text-sm text-gray-800 border rounded p-3 h-48 overflow-auto bg-white">
          {logs.length === 0 ? (
      ...
      <div>
        ...
        {!result ? (
          ...
          <div className="text-xs bg-gray-900 text-gray-200 border border-gray-800 rounded p-3 whitespace-pre-wrap">
            <pre>{pretty(result)}</pre>
            {result?.artifacts && (
              <div className="mt-2 text-gray-300">
                {result.artifacts.lesson_path && (
                  <div>Lesson JSON: <code className="text-gray-200">{result.artifacts.lesson_path}</code></div>
                )}
                {result.artifacts.notebook_path && (
                  <div>Notebook: <code className="text-gray-200">{result.artifacts.notebook_path}</code></div>
                )}
                {result.artifacts.smoke_report_path && (
                  <div>Smoke Report: <code className="text-gray-200">{result.artifacts.smoke_report_path}</code></div>
                )}
